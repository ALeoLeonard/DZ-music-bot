...
var Spotify = Spotify || {
  ...
  getSong: () => {
    ...
    .then((data) => {
      ...
      return Spotify.api.getPlaylistTracks('122506641', '1G8x5WyYJx7kUD8FjWDLzl')
    }).then((data) => {
      var songs = data.body.items
          ...
          songPick = {
            ...
            uri: `https://open.spotify.com/track/${song.track.id}`,
            artists: song.track.artists.map((artists) => {return artists['name']})
          };
  ...
  */
  tweet: (songPick) => {
    ...
    TwitBot.api.post('statuses/update', {
      status: `${songPick.name} BY ${artists} ${songPick.uri} #DZsTunez`
    }, (err, data, response) => {
      if (err){
        console.log(err);
      }
